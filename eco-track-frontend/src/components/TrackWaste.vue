...
const records = ref(JSON.parse(localStorage.getItem('wasteRecords') || '[]'))

onMounted(() => {
  // Update pinia store when component loads
  wasteStore.setWasteItems(records.value)
})

watch(records, (newVal) => {
  localStorage.setItem('wasteRecords', JSON.stringify(newVal))
  wasteStore.setWasteItems(newVal)
}, { deep: true })
